...
class Jobs extends React.Component {
    ...
    loadJobs() {
        ...
        Storage.get('userId').then((value) => {
            ...
            Api.get('completedjobs?userId=' + userId).then(function (response) {
                ...
                Storage.set('completedJobsList', JSON.stringify(response.data)).then(() => {
                    console.log('added jobs to storage')
                });

            }).catch((err) => {
                if (err.message == 'Network request failed') {
                    Storage.get('completedJobsList').then((data) => {
                        that.setState({
